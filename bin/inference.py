...
class TISSUE_FTU(Enum):
    ...
    LUNG = ("lung", "alveolus")
    LARGE_INTESTINE = ("largeintestine", "crypt")
    PROSTATE = ("prostate", "glandular acinus")
    SPLEEN = ("spleen", "white pulp")
...
def my_collate_fn(batch):
    ...
    orig_h = (
        []
    )  # Tile size (resolution without resize); equal to self.sz = config['resolution']
    orig_w = (
        []
    )  # Tile size (resolution without resize); equal to self.sz = config['resolution']
    organ = []
...
class Timm_Unet(nn.Module):
    def __init__(
        ...
        inp_size=3,
        otp_size=1,
        decoder_filters=[32, 48, 64, 96, 128],
        config=None,
        **kwargs,
    ):
        ...
        if name.startswith("coat"):
            ...
            encoder = timm.create_model(
                name, features_only=True, pretrained=pretrained, in_chans=inp_size
            )
    ...
    def forward(self, x):
        ...
        x1 = torch.cat(
            [
                F.adaptive_avg_pool2d(enc5, output_size=1).view(batch_size, -1),
    ...
    params = [
        {
            ...
            "models": [
                (
                    ...
                    "tf_efficientnet_b7_ns_768_e34_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "convnext_large_384_in22ft1k",
                    "convnext_large_384_in22ft1k_768_e37_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "tf_efficientnetv2_l_in21ft1k",
                    "tf_efficientnetv2_l_in21ft1k_768_e36_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "coat_lite_medium",
                    "coat_lite_medium_768_e40_{}_best",
                    models_folder,
                    3,
                ),
            ],
            "pred_dir": "test_pred_768",
            "weight": 0.2,
        },
        {
            "size": (1024, 1024),
            "models": [
                (
                    "convnext_large_384_in22ft1k",
                    "convnext_large_384_in22ft1k_1024_e32_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "tf_efficientnet_b7_ns",
                    "tf_efficientnet_b7_ns_1024_e33_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "tf_efficientnetv2_l_in21ft1k",
                    "tf_efficientnetv2_l_in21ft1k_1024_e38_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "coat_lite_medium",
                    "coat_lite_medium_1024_e41_{}_best",
                    models_folder,
                    3,
                ),
            ],
            "pred_dir": "test_pred_1024",
            "weight": 0.3,
        },
        {
            "size": (1472, 1472),
            "models": [
                (
                    "tf_efficientnet_b7_ns",
                    "tf_efficientnet_b7_ns_1472_e35_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "tf_efficientnetv2_l_in21ft1k",
                    "tf_efficientnetv2_l_in21ft1k_1472_e39_{}_best",
                    models_folder,
                    1,
                ),
                (
                    "coat_lite_medium",
                    "coat_lite_medium_1472_e42_{}_best",
                    models_folder,
                    3,
                ),
            ],
            "pred_dir": "test_pred_1472",
            "weight": 0.5,
        },
    ]
    return params
...
def load_models(param, inference_mode, config):
    ...
    for model_name, checkpoint_name, checkpoint_dir, model_weight in param["models"]:
        ...
        if inference_mode == "fast":
            ...
            models.append((model, model_weight))
            print(
                "Fast Inference Mode: loaded (model {}, fold {}, best_score {})".format(
                    model_name, best_fold, best_score
                )
            )
    return models
...
def predict_models(param, im_path, organ, inference_mode, config):
    ...
    with torch.inference_mode():
        ...
        pred_mask = np.zeros(
            (len(test_data), test_data.pred_sz, test_data.pred_sz), dtype=np.uint8
        )
        ...
        for sample in tqdm(test_data_loader):
            ...
            with amp_autocast():
                for _tta in range(4):  # 8
                    ...
                    for model, model_weight in models:
                        ...
                        pred_mask_float += model_weight * msk_pred[:, 0, :, :].astype(
                            "float32"
                        )
                del inp
                torch.cuda.empty_cache()
            ...
            pred_mask_float = np.vstack(
                [
                    cv2.resize(_mask.astype(np.float32), (orig_w[0], orig_h[0]))[None]
                    for _mask in msk_pred_scaled
                ]
            )
            pred_mask_int = (
                pred_mask_float > organ_threshold[data_source][organ]
            ).astype(np.uint8)
            ...
            for j in range(bs):
                ...
                pred_mask[st_ind + j, 0 : py1 - py0, 0 : px1 - px0] = pred_mask_int[
        ...
        pred_mask = pred_mask.reshape(
            test_data.num_h * test_data.num_w, test_data.pred_sz, test_data.pred_sz
        ).reshape(
            test_data.num_h, test_data.num_w, test_data.pred_sz, test_data.pred_sz
        )
        pred_mask = pred_mask.transpose(0, 2, 1, 3).reshape(
            test_data.num_h * test_data.pred_sz, test_data.num_w * test_data.pred_sz
        )

        pred_mask = pred_mask[
            : test_data.h, : test_data.w
        ]  # back to the original slide size
...
def mask2json(mask, organ):
    ...
    for i, polygon in enumerate(contours):
        ...
        geojson_dict["geometry"]["coordinates"].extend(
            [[x[0] for x in polygon.tolist()] + [polygon.tolist()[0][0]]]
        )
...
def main(
    ...
    tissue_ftu: TISSUE_FTU = tissue_code_mapping[tissue_code]
    ...
    for image_path in image_paths:
        ...
        preds = []
        im_path = image_path
        organ = str(tissue_ftu)

        print(f"Image Path is: {image_path} and stem is: {path_stem}")

        for param in params:
            img_pred = predict_models(
                param, im_path, organ, args.inference_mode, config
            )
        ...
        print(
            "Mask min,max values before thresholding",
            np.min(pred_mask),
            np.max(pred_mask),
        )
        ...
        OmeTiffWriter.save(
            pred_mask_thr, output_dir / f"{path_stem}.segmentations.ome.tiff"
        )

        json_mask = mask2json(pred_mask_thr, organ)
        with open(output_dir / f"{path_stem}_mask.json", "w") as f:
