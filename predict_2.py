...
def get_model_list(model_name, checkpoint_name, checkpoint_dir, model_weight):
    models=[]
    for fold in range(5):
...
def predict_models(param,im_path,organ,data_source):
    ...
    for model_name, checkpoint_name, checkpoint_dir, model_weight in param['models']:
        models = get_model_list(model_name, checkpoint_name, checkpoint_dir, model_weight)
    torch.cuda.empty_cache()
    with torch.no_grad():
            im_path = im_path
            ...
            for sample in tqdm(test_data_loader):
                ...
                with amp_autocast():
                    for _tta in range(4): #8
                        _i,_flip = get_i_flip(_tta)
                        inp = preprocess(imgs,_i,_flip)                  
                        torch.cuda.empty_cache()
                        for model, model_weight in models:
                            msk_preds,cnt = forward_pass(model,model_weight,_i,_flip,cnt,msk_preds,inp,ids,orig_w,orig_h)
                        del inp
                        torch.cuda.empty_cache()
                ...
                msk_stack_int = (msk_stack>organ_threshold[data_source][organ]).astype(np.uint8)
    ...
    return pred_mask, pred_dir, fname
            ...
            #cv.imwrite(f'/u/athbagde/hubmap_solution_victor/op_im/{pred_dir}_{fname}.png',pred_mask, [cv2.IMWRITE_PNG_COMPRESSION, 4])

def forward_pass(model,model_weight,_i,_flip,cnt,msk_preds,inp,ids,orig_w,orig_h):
    ...
    return msk_preds,cnt
    
def preprocess(imgs,_i,_flip):
...
for ind,row in test_df.iterrows():
    ...
    print(pred)
    OmeTiffWriter.save(pred, f'/u/athbagde/hubmap_solution_victor/op_im/{pred_dir}_{fname}.ome.tif')
    ...
    with open(f'/u/athbagde/hubmap_solution_victor/op_im/colon/{pred_dir}_{fname}.json', "w") as outfile:
            json.dump(json_dir,outfile)
    outfile.close()
